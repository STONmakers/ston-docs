...
1. Web Cache
============
Web Cache는 클라이언트에 가까운 (Edge) 위치에서 컨텐츠를 전달하는 서버이다.

통상 다음과 같은 동작을 기본으로 한다.

1. 클라이언트로부터 컨텐츠의 최초 요청을 받고
2. 컨텐츠를 원본서버에서 가져와 저장하여 클라이언트에게 다시 전송한다. 
3. 컨텐츠 재요청을 받으면 가져왔던 컨텐츠를 즉시 전송한다. 

다양한 통신과 연산을 요구받는 일반적인 웹서버와 달리, 
정적 컨텐츠 전송에 특화되어 다수 클라이언트에게 동일 컨텐츠를 전송하는 서비스에 매우 효과적이다.
