...
1. Web Cache
============
Web Cache는 서비스 네트워크의 최전방 (Edge)에서 클라이언트에게 컨텐츠를 전달하는 서버로, 아래와 같은 기본 동작구조를 가진다.
...
Web Cache를 도입하여 설치 및 적용부터 운영에 이르는 과정까지, 가장 많이 조작하는 담당자는 보통 운영자다. 

STON은 기획단계부터 서비스 운영자 (SE, System Engineer)의 관점에 맞추어 설계되었다. 

프로그래밍 랭귀지를 습득할 필요도 없으며, 복잡하고 어려운 코딩을 할 필요도, 변경설정 적용을 위해 빌드를 다시 할 필요도 없다.
