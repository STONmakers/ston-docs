...
Web Cache는 서비스 네트워크의 최전방(Edge)에서 클라이언트에게 컨텐츠를 전달하는 서버로, 
다음과 같은 기본 동작구조를 가진다.
...
그러나 Web Cache를 도입하면 원본 웹서버는 그대로 유지하면서 Web Cache 추가증설만으로 손쉽게 서비스를 확장할 수 있다. 
원본 웹서버의 컨텐츠만 갱신하면, Web Cache가 자동으로 컨텐츠를 가져다가 전송한다.
