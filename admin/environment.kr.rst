...
-  ``<Storage>``
    콘텐츠를 저장할 디스크를 설정한다. 
    디스크 개수에 제한은 없다. 
    각 디스크마다 최대 캐싱용량(Quota, 단위: GB)을 설정할 수 있다. 
    ``Quota`` 를 설정하지 않아도 디스크가 꽉 차지 않도록 LRU(Least Recently Used) 알고리즘에 의해 오래된 콘텐츠를 자동으로 삭제한다.
    ...
    모든 디스크가 배제될 수도 있는데 이 때의 동작방식은 ``OnCrash`` 속성을 통해 설정한다.
