...
이번 장에서는 콘텐츠 서비스의 핵심이 되는 Caching-Key와 만료정책에 대해 설명한다.
HTTP에는 콘텐츠를 구분하는 다양한 규격이 존재한다. 
그만큼 Caching-Key도 다양하게 존재할 수 있다.

콘텐츠 변경가 없을수록 원본부하를 줄일 수 있을뿐만 아니라 쉽게 확장할 수 있다.
서비스에 최적화된 만료정책을 수립하는 다양한 방식에 대해 설명한다.

이장에서 설명되는 설정을 모든 가상호스트의 기본 설정으로 적용하고 싶다면 ``<VHostDefault>`` 하위에 설정한다.
...
같은(또는 다른) 콘텐츠가 캐싱될 수 있다. 원본서버에 요청을 보내는 시점에 
...
QueryString에 의하여 동적으로 생성되는 콘텐츠가 아니라면 QueryString을 인식하는 것은 
...
.. note::
    ...
    많은 메모리가 필요해질 수 있다. 가능한 작게 설정하는 것이 좋다.
...
-  ``<Res4xx> (기본: 30초)``
    원본서버가 4xx로 응답했을 때 TTL을 설정한다. 404 Not Found인 경우가 많다.
...
-  ``<ConnectTimeout> (기본: 3초)``
    ...
    이는 장애상황을 서비스한다는 의미보다는 TTL시간동안 (아마도 장애상황일) 원본서버에 
    부담을 주지 않는 의미가 강하다.
...
예를 들어 콘텐츠를 저장하는 Storage와의 연결을 잃거나, 개발적인 문제로 정상처리가 불가능하다고 판단하는 경우가 대표적이다.
전자의 경우 4xx응답(주로 404 Not Found), 후자는 5xx응답(주로 500 Internal Error)을 받게된다.
하지만 이미 STON에 관련 콘텐츠가 저장되어 있다면, 원본의 응답을 믿는 것보다 
TTL을 연장시켜 서비스 전체장애가 발생하지 않도록 할 수 있다. ::
...
원본서버에 의해 의도된 4xx응답인 경우 의도하지 않은 결과를 응답할 수 있으니 주의가 필요하다.
       
-  ``<TTLExtensionBy5xx>``

   -  ``ON (기본)`` 304 not modified를 받은 것처럼 동작한다.
   
   -  ``OFF`` 5xx 응답으로 콘텐츠를 갱신한다.


정상적인 서버라면 5xx로 응답하지 않는다. 주로 서버의 일시적인 장애로부터 
콘텐츠를 무효화하여 원본부하를 가중시키지 않기 위한 용도로 사용된다.
...
고화질 동영상이나 게임 클라이언트처럼 상대적으로 반응성보다 전송속도가 중요한 
서비스에서는 이런 방식이 큰 문제가 되지 않는다. 
대용량 데이터의 경우 원본서버가 10초만에 갱신에 대한 응답을 한다고 하더라도 
전송에 몇 분씩 소요되기 때문에 상대적으로 원본의 반응성이 크게 중요하지 않다. 
오히려 접근 빈도가 높지 않은 콘텐츠이기 때문에 반드시 갱신확인이 이루어져야 한다.

하지만 쇼핑몰과 같은 경우 상황은 다르다. 클라이언트에게는 웹 페이지가 빠르게 
로딩되는 것이 무엇보다도 중요하다. 
웹 페이지의 모든 리소스는 1~2초 안에 서비스가 되어야 한다. 
전송속도보다 반응속도가 더 중요하다는 말이다. 
이 때 TTL이 만료되어 원본서버에게 갱신확인을 해야 한다면 매우 큰 지연이 
발생할 수 있다. 보통의 쇼핑몰이 수백만개의 콘텐츠를 동시에 서비스 하는 것을 
감안할 때 항상 원본서버로부터 갱신확인 작업이 발생하고 있다고 생각해야 한다. 
이때 원본서버 장애가 발생하거나 네트워크 장애가 발생한다면 최악이다.

우리가 원하는 것은 어떠한 원본서버의 장애나 지연으로부터 캐싱된 콘텐츠가 
안전하게 전송되는 것이다.
...
``OFF`` 설정의 더 큰 이유는 콘텐츠가 대부분 자주 바뀌지 않기 때문이다.
   ...
   .. figure:: img/perf_refreshexpired5.jpg
      ...
      변경에 민감하지 않다면 기다리는 시간을 없앨 수 있다.

위 그림에서 원본서버와의 갱신작업이 모두 백그라운드로 이루어지기 때문에 캐싱된 
콘텐츠는 기다림없이 즉시 클라이언트에게 서비스된다. 
원본서버가 304 Not Modified로 응답한다면 TTL만 연장된다. 
파일이 갱신되어 원본서버에서 200 OK를 응답한 경우 해당 파일이 완전히 다운로드된 
후에 파일이 Seamless하게 교체된다. 
콘텐츠이 바뀌어도 이전 콘텐츠(초록색)를 다운로드 받던 사용자들은 정상적으로 
