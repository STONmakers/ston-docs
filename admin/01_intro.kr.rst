...
STON은 관리자를 위해 존재한다.
관리자에 의해 운영되고, 관리자에 의해 검증되고, 관리자에 의해 개선되는 제품이 되는 것이다. 
STON개발팀이 이루고 싶은 가치는 고객, 관리자와 함께 더 나은 서비스를 만드는 것입니다.
...
하지만 그렇게 개발해봤자 베낀 소스의 Copy Cat밖에 되지 못하지 않은가. 
서두에서 언급한 바와 같이 "~같은 제품"이 아닌 "관리자를 위한~" 솔루션을 만들고 싶었다. 

서비스에서 장애는 필연적이다. 
사람과 마찬가지로 솔루션도 실패를 먹고 자란다. 
하지만 소스가 온전히 내 것이 아니라면 실패를 기반으로 성장할 수 없다는 점을 알고 있다. 
...
가장 정직하지만 가장 근본적인 해결책은 처음부터 끝까지 모두 직접 개발하는 것다.
다른 방법은 없다.
...
쇠가 빨개졌을 때 두드려야 원하는 결과물을 얻을 수 있는 것과 같은 이치다. 
...
처음엔 Erlang을 고려했었다. 
안정성과 무중단 서비스가 솔루션의 컨셉과 잘 맞는다고 느꼈기 때문이다. 
...
쓸데 없이 설정파일이 길어지는 것도 피하고자 한다. 
고객이 어려워할 수 있는 요소들은 최대한 배제하려 한다.
...
우리는 고객으로부터 요청된 기능은 거의 대부분 개발한다. 
