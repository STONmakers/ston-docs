...
-  **메모리**
   ...
   1,200만개의 제한을 두는 이유는 Disk I/O의 성능저하가 우려되기 때문입니다. 
   파일이 많지 않더라도 파일 크기가 커서 Disk 병목현상이 발생한다면 메모리를 
   증설하는 것이 좋은 해결책입니다. 메모리에 로딩된 컨텐츠는 
   LRU(Least Recently Used)원칙에 따라 유지됩니다. 
...
- **디스크**
   ...
   항상 Write작업이 있다고 생각하시는 편이 좋습니다. OS도 디스크 I/O를 사용하지만 
   미비한 수준이며 시스템 메모리 페이징(Paging)이 발생하지 않는다면 디스크 증설보다는 
   메모리를 증설하시는 것이 좋습니다. STON은 기본적으로 디스크를 RAID 0처럼 사용합니다. 
