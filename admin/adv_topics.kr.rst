...
앞의 그림에서 쉽게 알 수 있는 부분은 생각보다 사용하지 않는(이하 Free) 메모리 공간이 크다는 점이다. 
Contents메모리가 핵심이라고 해놓고 사용하지 않는 영역이 크다니 다소 이상해 보인다. 
결론적으로 아무리 많은 Contents메모리를 사용한다고 하더라도 OS가 느리면 빠르게 서비스를 할 수 없다. 
...
특별히 최대 파일개수를 지정하지 않는다면 위의 표만큼 파일개수를 유지하며 메모리를 사용한다.
이 설정은 쇼핑몰과 같이 평균 파일크기는 4KB이하로 작지만 개수가 많은 서비스에 적합하다. 
...
쇼핑몰과 같이 파일이 많은 서비스가 아니라면 최대 파일개수를 보다 적게 설정하는 것이 좋다. 
...
또 하나의 중요한 변수는 클라이언트 Keep-Alive시간 설정이다.
...
STON Contents영역을 최대한 확보하기 위해 조절할 수 있는 변수는 다음과 같다.
...
============= ============== =============== ======= ================= =============
물리 메모리   System Free    최대 파일개수   Core    최초 Socket개수   STON Contents
============= ============== =============== ======= ================= =============
