...
예를 들어 STON이 3만개의 세션을 처리할 때 원본 서버들이 모두 한계에 다다른다면 
이 수치를 3~4만 정도로 설정하는 것이 좋다. 
이로 인해 얻을 수 있는 효과는 다음과 같다.

-  별다른 Network 구성(i.e. L4등)이 필요 없다.
-  원본 서버에서 추가적인 부하 대처를 할 필요가 없다.
-  불필요한(원본 부하로 처리될 수 없는) 클라이언트 요청을 방지한다.
-  STON의 신뢰성을 높인다. 서비스 Burst 이후 재시작 등 점검 작업이 필요 없다.
...
HTTP 클라이언트 연결을 처리하기 위한 세션 수를 설정한다. ::
...
할당된 소켓이 모두 사용되면 자동으로 소켓을 증설한다. 
이때 별도의 ``<TopUp>`` 을 지정하지 않았다면 Max의 10%만큼 소켓을 늘린다. 
``<Max>`` 개수만큼 소켓이 사용된다고 하여도 클라이언트가 증가하면 소켓은 계속 늘어난다.

.. note::

   Cent OS 5.5이상과 Ubuntu 10이상에서 동시접속 소켓이 10만을 넘으면 시스템 성능이 저하되며 소켓처리가 실패하는 증상이 있다. 
   그러므로 최대 소켓개수를 10만 이상으로 설정하여도 10만으로 제한된다.
